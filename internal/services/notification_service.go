...
func (s *NotificationService) SaveNotification(ctx context.Context, reportID, reportDate, submittedBy, notes string) {
	...
	if err != nil {
		slog.Error("SaveNotification: failed to check existing notification", "reportID", reportID, "error", err)
		return
	}
	...
	if cleanNotes == "" {
		if existing != nil {
			...
			slog.Info("SaveNotification: removed empty notification for report", "reportID", reportID)
		}
		return
	}
	...
	if existing != nil {
		...
		if err := s.repo.Update(bgCtx, existing); err != nil {
			slog.Error("SaveNotification: failed to update notification", "reportID", reportID, "error", err)
		} else {
			slog.Info("SaveNotification: updated notification", "reportID", reportID, "date", reportDate)
		}
		return
	}
	...
	if err := s.repo.Create(bgCtx, notif); err != nil {
		slog.Error("SaveNotification: failed to create notification", "reportID", reportID, "error", err)
	} else {
		slog.Info("SaveNotification: created notification", "reportID", reportID, "date", reportDate)
	}
}
